...
### Data Collection
- **Dual Market Profile Support**: Supports currently active BTC Up/Down markets for both 5-minute (`btc_5m`) and 15-minute (`btc_15m`) profiles.
...
Use one of the following commands when starting the logger:
...
Storage paths for each runtime profile:
...
- **Phase 1 (fast scout)** tests only `(minutes_after_open, entry_threshold)` with second-entry forced `off`.
- **Phase 2 (refine)** takes the best phase-1 `(minutes_after_open, entry_threshold)` pairs and sweeps second-entry threshold + second-entry mode.
...
| `target_time_uk` | Market start time in UK time (profile window start: 15m or 5m). |
| `expiration_uk` | Market expiration time in UK time (profile window end: 15m or 5m). |
...
If you do not provide `--market-type`, the logger uses the current code default: **`btc_15m`** (`default_market_profile_key()`).
